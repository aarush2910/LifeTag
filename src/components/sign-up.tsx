...
export default function SignPage() {
  ...
  // helper to persist login info after successful login
  const persistLogin = (loginData: any, roleLocal: string, loginIdentifier: string) => {
    try {
      ...
      if (roleLocal === "farmer") {
        ...
        localStorage.setItem("faadhar", loginIdentifier);
      } else if (roleLocal === "vet") {
        localStorage.setItem("identifier", loginIdentifier);
        localStorage.setItem("vemail", loginIdentifier);
      } else {
        localStorage.setItem("identifier", loginIdentifier);
        localStorage.setItem("semail", loginIdentifier);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/auth/signup/${role}`, {
        method: "POST",
      ...
      // Signup succeeded. Now auto-login to get JWT and set identifier.
      let loginIdentifier = "";
      if (role === "farmer") loginIdentifier = faadhar.replace(/\s/g, "");
      else if (role === "vet") loginIdentifier = vemail.trim();
      else loginIdentifier = semail.trim();
      ...
      try {
        ...
        if (!loginRes.ok) {
          // Signup success but login failed — navigate user to login page
        ...
        persistLogin(loginData, role, loginIdentifier);
    ...
    <section className="flex min-h-screen bg-gradient-to-br from-zinc-50 via-zinc-100 to-zinc-50 px-4 py-16 md:py-24 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950">
      <form onSubmit={handleSubmit} autoComplete="off" className="bg-card m-auto h-fit w-full max-w-2xl rounded-2xl border shadow-2xl dark:[--color-muted:var(--color-zinc-900)] overflow-hidden">
        ...
        <div className="p-8">
          <AnimatePresence>
            {error && (
              <motion.div initial={{ opacity: 0, y: -20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.95 }} transition={{ duration: 0.4 }} className="mb-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 shadow-lg">
                <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 400, damping: 20 }} className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-red-800 dark:text-red-200">
                    {Array.isArray(error) ? error.map((msg, idx) => <div key={idx} className="mb-1 last:mb-0">• {msg}</div>) : error}
                  </div>
                </motion.div>
              </motion.div>
            )}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input type="password" required id="pwd" name="new-password" autoComplete="new-password" className="h-11" placeholder="Create a strong password" value={password} onChange={(e) => setPassword(e.target.value)} />
